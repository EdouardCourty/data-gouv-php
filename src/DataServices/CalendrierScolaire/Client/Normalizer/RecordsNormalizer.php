...
class RecordsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \Ecourty\DataGouv\DataServices\CalendrierScolaire\Client\Model\Records::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \Ecourty\DataGouv\DataServices\CalendrierScolaire\Client\Model\Records::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        $object = new \Ecourty\DataGouv\DataServices\CalendrierScolaire\Client\Model\Records();
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('records') && null !== $data->getRecords()) {
            $values_1 = [];
            foreach ($data->getRecords() as $value_1) {
                $values_1[] = $value_1;
            }
            $dataArray['records'] = $values_1;
        }
        foreach ($data as $key => $value_2) {
            if (preg_match('/.*/', (string) $key)) {
    ...
    {
        return [\Ecourty\DataGouv\DataServices\CalendrierScolaire\Client\Model\Records::class => false];
    }
}