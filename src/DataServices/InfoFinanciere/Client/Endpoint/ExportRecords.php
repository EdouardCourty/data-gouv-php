...
class ExportRecords extends \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Runtime\Client\BaseEndpoint implements \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Runtime\Client\Endpoint
{
    ...
    * } $queryParameters
    */
    public function __construct(string $datasetId, string $format, array $queryParameters = [])
    {
    ...
    {
        return str_replace(['{dataset_id}', '{format}'], [$this->dataset_id, $this->format], '/catalog/datasets/{dataset_id}/exports/{format}');
    }
    ...
    protected function getQueryOptionsResolver(): \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(['select', 'where', 'order_by', 'limit', 'refine', 'exclude', 'lang', 'timezone', 'use_labels', 'compressed', 'epsg']);
        $optionsResolver->setRequired([]);
        $optionsResolver->setDefaults(['limit' => -1, 'timezone' => 'UTC', 'use_labels' => false, 'compressed' => false, 'epsg' => 4326]);
        ...
        $optionsResolver->addAllowedTypes('use_labels', ['bool']);
        $optionsResolver->addAllowedTypes('compressed', ['bool']);
        $optionsResolver->addAllowedTypes('epsg', ['int']);
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Exception\ExportRecordsUnauthorizedException($response);
        }
        if (429 === $status) {
        }
        if (500 === $status) {
            throw new \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Exception\ExportRecordsInternalServerErrorException($response);
        }
    }
