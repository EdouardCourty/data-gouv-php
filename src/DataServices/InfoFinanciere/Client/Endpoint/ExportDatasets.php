...
class ExportDatasets extends \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Runtime\Client\BaseEndpoint implements \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Runtime\Client\Endpoint
{
    protected $format;
    /**
    ...
    * } $queryParameters
    */
    public function __construct(string $format, array $queryParameters = [])
    {
        $this->format = $format;
    ...
    {
        return str_replace(['{format}'], [$this->format], '/catalog/exports/{format}');
    }
    ...
    protected function getQueryOptionsResolver(): \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(['select', 'where', 'order_by', 'limit', 'offset', 'refine', 'exclude', 'lang', 'timezone']);
        $optionsResolver->setRequired([]);
        $optionsResolver->setDefaults(['limit' => -1, 'offset' => 0, 'timezone' => 'UTC']);
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Exception\ExportDatasetsUnauthorizedException($response);
        }
        if (429 === $status) {
        }
        if (500 === $status) {
            throw new \Ecourty\DataGouv\DataServices\InfoFinanciere\Client\Exception\ExportDatasetsInternalServerErrorException($response);
        }
    }
